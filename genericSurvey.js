...
const $=id=>document.getElementById(id),screens=[...document.querySelectorAll(".survey-screen")],total=screens.length;
...
async function submit(){if(submitting)return;submitting=true;let b=$("nextBtn");b.disabled=true;b.innerHTML='<span class="submit-loading"><span class="submit-spinner"></span>Submitting…</span>';try{await fetch(APPS_SCRIPT_URL,{method:"POST",mode:"no-cors",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(payload())})}catch(e){submitting=false;b.disabled=false;b.innerHTML='Submit survey <b>✓</b>';return}document.querySelectorAll(".survey-screen").forEach(s=>s.hidden=true);$("thanks").hidden=false;$("footer").hidden=true;$("responseId").textContent=state.responseId;$("backBtn").style.visibility="hidden";$("screenNo").textContent="✓";$("progress").style.width="100%"}
$("startBtn").onclick=()=>{$("intro").hidden=true;$("survey").hidden=false;showStep(1)};$("backBtn").onclick=()=>{if(state.step>1)showStep(state.step-1)};$("nextBtn").onclick=()=>{if(state.step<total)showStep(state.step+1);else submit()};bindOptions();showStep(1);
